...
def stage_backward_input(
        ...
        for i, intermediate in enumerate(param_group["intermediates"]):
            ...
            def get_hook(param_group, i):
                def hook(grad_inputs):
                    grads = param_group.setdefault("grads", [None] * len(param_group["intermediates"]))
                    grads[i] = tuple(g.detach() for g in grad_inputs)
    ...
    for i, inp in enumerate(input_values):
        if inp.grad is None:
            ...
            inp.grad.add_(dinputs[i])  # CSH: in-place addition
            # inp.grad += dinputs[i]
...
def stage_backward_weight(
    ...
    for index, weight in enumerate(weights):
        if weight.requires_grad:
            ...
            assert (grad_acc is not None), f"Weight {weight} requires grad but has no grad accumulator. "
    ...
    for param_group in param_groups:
        ...
        weights_edges = tuple(GradientEdge(w, 0) for w in param_group["params"] if w.requires_grad) # CSH

        if len(grad_acc_to_weight) == 0 or len(weights_edges) == 0 or len(intermediate_edges) == 0:
            # CSH: No intermediates or weights to update, skip this param_group
        ...
        for grad_acc, dw in zip(param_group["params"], dweights):
            if not grad_acc in grad_acc_to_weight.keys(): # CSH : skip if grad_acc is None
                continue
            weight, index = grad_acc_to_weight[grad_acc]
            if weight.grad is None:
                weight.grad = dw
            else:
                weight.grad = weight.grad.add_(dw)
...
def stage_backward(
    stage_output: List[torch.Tensor],
    output_grads: Optional[List[torch.Tensor]],
    input_values: List[torch.Tensor],
    weights: Iterator[Parameter],
    ...
    try:
        ...
        # CSH: set requires_grad=True for all input values
        for val in input_values:
            val.requires_grad_()

        torch.autograd.backward(
            stage_output_tensors, grad_tensors=output_grad_tensors,  # type: ignore[arg-type]
            inputs=input_values + [param for param in weights if param.requires_grad] # CSH
        )
