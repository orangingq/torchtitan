...
class _PipelineStageBase(ABC):
    ...
    def __init__(
        ...
        # CSH: map microbatch ID to the gradient of the parameters
        self.param_grad_cache: Dict[str, List[torch.Tensor]] = {} 
    ...
    def set_param_requires_grad(self, requires_grad: bool = True):
        """CSH: Set the requires_grad attribute for all parameters in the submodule."""
        if len(self.param_grad_cache) > 0: # already set
            assert all(param.requires_grad == requires_grad for param in self.submod.parameters()), "Requires_grad already set. Please reset it first."
            return
        for name, param in self.submod.named_parameters():
            self.param_grad_cache[name] = param.requires_grad
            param.requires_grad_(requires_grad)
        return
    
    def reset_param_requires_grad(self):
        """CSH: Reset the requires_grad attribute for all parameters in the submodule."""
        if len(self.param_grad_cache) == 0: # already reset
            return
        for name, param in self.submod.named_parameters():
            param.requires_grad_(self.param_grad_cache.pop(name))
        self.param_grad_cache.clear()
        return
    ...
    def forward_maybe_with_nosync(self, *args, **kwargs):
        ...
        self.set_param_requires_grad(True) #CSH
        ...
        self.reset_param_requires_grad() #CSH
        return out_val
    ...
    def backward_one_chunk(
        ...
        if self.is_last and not self.is_first:
            ...
            for t in stage_output:
                t.detach_() #CSH : changed from .detach_() to detach() to avoid in-place operation warning
