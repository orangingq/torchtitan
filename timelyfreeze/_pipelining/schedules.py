...
class ScheduleInterleavedZeroBubble(PipelineScheduleMulti):
    ...
    def _add_bubbles_to_actions(self, num_stages_global):
        """ Add bubbles to the actions based on dependencies of actions."""
        actions = self.pipeline_order

        def need_bubble(stage, op, microbatch, num_stages_global, seen_ops):
            if op == _ComputationType.FORWARD: # If current op is forward, check if previous stage's forward op is seen (i.e., has been executed)
                if stage != 0 and (stage - 1, op, microbatch) not in seen_ops:
                    return True
            elif op == _ComputationType.FULL_BACKWARD: # If current op is backward, check if forward op is seen and if the next stage's backward op is seen (i.e., has been executed)
...
class ScheduleZBVZeroBubble(PipelineScheduleMulti):
    ...
    def step(self, *args, target=None, losses: Optional[List] = None, **kwargs):
        ...
        # CSH: Parameter propagation
        self.gradient_sync_aft_optimizer()

        # Run microbatches
        self._step_microbatches(args_split, kwargs_split, targets_split, losses)
        
        # CSH: Gradient sync for zbv schedule
        self.gradient_sync_bef_optimizer(losses)
    ...
    def gradient_sync_bef_optimizer(self, losses:List[float])->float:
        """Gradients Synchronization between stages before optimizer step."""

        # average gradients between stages
        ops: List[dist.P2POp] = []
        peer=self.pp_group_size-1-self.rank
        sender = self.rank if self.rank < self.pp_group_size // 2 else peer # pp rank 0 ~ pp_group_size//2-1 : send model partitions to the peer rank
        receiver = peer if self.rank < self.pp_group_size // 2 else self.rank # pp rank self.pp_group_size//2 ~ pp_group_size-1 : receive model partitions from the peer rank and sync gradients
        
        # pp rank 0 ~ self.pp_group_size//2-1 : send model partitions to the peer rank
        if self.rank == sender: 
            for i in [1, 0]: # send model 1 and model 0 parameters
                for name, param in self._stages[i].named_parameters():
                    # send model parameter gradients
                    tensor = torch.zeros_like(param) if param.grad is None else param.grad.data
                    ops.append(dist.P2POp(op=dist.isend, tensor=tensor, peer=peer))
            if self._stages[0].is_first:
                # send the loss to the last stage
                ops.append(dist.P2POp(op=dist.isend, tensor=torch.stack(losses), peer=peer))
        
        # pp rank self.pp_group_size//2 ~ self.pp_group_size-1 : receive model partitions from the peer rank
        else: 
            received_params = {name: torch.zeros_like(param) for i in [0, 1] for name, param in self._stages[i].named_parameters()}
            for i in [0, 1]:
                for name, param in self._stages[i].named_parameters():
                    # receive model parameter gradients
                    ops.append(dist.P2POp(op=dist.irecv, tensor=received_params[name], peer=peer))
            if self._stages[0].is_last:
                # receive the loss from the first stage
                received_params['loss'] = torch.zeros_like(torch.stack(losses))
                ops.append(dist.P2POp(op=dist.irecv, tensor=received_params['loss'], peer=peer))
        ...
        if self.rank == receiver: 
            ...
            for i in [0, 1]:
                for name, param in self._stages[i].named_parameters():
                    # average of the orginal gradients and the received gradients
                    if torch.count_nonzero(received_params[name]) > 0:
                        param.grad = (received_params[name] + param.grad) / 2 if param.grad is not None else received_params[name]

            if self._stages[0].is_last: # average the loss
                new_losses = (torch.stack(losses) + received_params['loss'])/2
                losses.clear()
                losses.extend([val for val in new_losses])
            del received_params
        return



    def gradient_sync_aft_optimizer(self):
        """Propagate updated parameter values from synced ranks to un-synced ranks."""

        # average gradients between stages
        ops: List[dist.P2POp] = []
        peer=self.pp_group_size-1-self.rank
        sender = peer if self.rank < self.pp_group_size // 2 else self.rank
        receiver = self.rank if self.rank < self.pp_group_size // 2 else peer
        
        # pp rank self.pp_group_size//2 ~ self.pp_group_size-1 : send model partitions to the peer rank
        if self.rank == sender: 
            for i in [1, 0]: # send model 1 and model 0 parameters
                for param in self._stages[i].parameters():
                    # send model parameter gradients
                    ops.append(dist.P2POp(op=dist.isend, tensor=param.data, peer=peer))
        
        # pp rank 0 ~ self.pp_group_size//2-1 : receive model partitions from the peer rank
        else: 
            # received_params = {name: torch.zeros_like(param) for i in [0, 1] for name, param in self._stages[i].named_parameters()}
            for i in [0, 1]:
                for param in self._stages[i].parameters():
                    # receive model parameter gradients
                    ops.append(dist.P2POp(op=dist.irecv, tensor=param.data, peer=peer))
