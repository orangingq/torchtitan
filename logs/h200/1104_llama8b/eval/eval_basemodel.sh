...
THIS_FILE="/opt/dlami/nvme/DMLAB/shcho/torchtitan/logs/h200/1104_llama8b/eval/eval_basemodel.sh" # "$(realpath "${BASH_SOURCE[0]}")"
...
for BASENAME in "${BASENAME_LIST[@]}"; do
    ...
    # Print the current timestamp and the server name
    {
        ...
        echo -e "✔️${EXPLAIN}"
        echo -e "☑️> python3 -m timelyfreeze.evaluation --model_path=${MODEL_PATH} --dtype=float16 --model_type=${BASENAME} --batch_size=16 --device_map=cuda --tasks=${TASKS} --output_json=${RESULT_FILE}"
